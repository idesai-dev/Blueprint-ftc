	...
	async function handleSubmit(e: Event) {
		...
		try {
			...
			if (response.ok && result.success) {
				...
				submitText = 'Error!';
				setTimeout(() => (submitText = 'Send CAD'), 3000);
			}
		} catch (error) {
			console.error(error);
			submitText = 'Error!';
...
<section class="review-section animate-fade-up">
	...
	{:else}
		<div class="review-content">
			<div class="text-side">
				<h2>CAD Review</h2>
				<p>Share your CAD models with us! We'll provide detailed feedback to help you <strong>perfect your robot design.</strong></p>
				<p class="reminder-box">
					<strong>Reminder:</strong> Give viewer access to <code class="email-code">ftcblueprint@gmail.com</code> before submitting.
				</p>
			</div>

			<div class="form-side">
				<form onsubmit={handleSubmit} class="review-form">
					<input type="hidden" name="access_key" value="77095db6-97a3-47d6-8768-b2e706abd2c2" />
					<input type="hidden" name="subject" value="New CAD Review Request" />
					...
					<input type="url" name="cad_link" placeholder="Onshape / Drive link to CAD" required />
					<textarea name="notes" placeholder="Notes (subsystems to focus on, specific concerns...)" rows="3"></textarea>
